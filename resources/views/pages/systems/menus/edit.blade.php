  ...
  <div class="col-md-6 mb-4 mb-md-0">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <form class="browser-default-validation" method="POST" action="{{ route('edit-menu', ['id' => $data->id]) }}">
             @csrf
            <div class="mb-3">
              <label class="form-label">Type</label>
              <select id="parent" name="parent" class="form-select">
                <option value="#" {{ ($data->parent == '0') ? "selected='selected'" : '' }}>Default Parent</option>
                @foreach ($menu_parents as $item)
                  <option value="{{ $item->id }}" {{ ($data->parent == $item->id) ? "selected='selected'" : '' }}>
                    {{ $item->name }}</option>
                @endforeach
              </select>
            </div>
...
@endsection
@section('add-js')
<script src="{{ asset('assets/js/pages/menu/edit.js') }}"></script>
@endsection
